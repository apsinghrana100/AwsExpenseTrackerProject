...
document.getElementById("premiusm-btn1").onclick=async function(event){
    ...
    try {
        ...
        const response=await axios.get("http://13.230.79.168:4000/purchasepremium", {headers:{'Authorization':token}});
        ...
        var options={
            ...
            "handler": async function(response){
             const resp=await axios.put("http://13.230.79.168:4000/updatepremium",
                {
        ...
        rzpl.on('payment.failed',async function(response){
            ...
            try {
                await axios.put("http://13.230.79.168:4000/premiumstatus",
                {
                    order_id:options.order_id,
                    payment_id:response.razorpay_payment_id,
