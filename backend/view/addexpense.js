...
selectElement.addEventListener("change", async () => {
  ...
  const expensedata=await axios.get(`http://13.230.79.168:4000/getexpensedata?param1=${pageno}&param2=${rowsize}`,{headers: {"Authorization":token}});
        showPagination(
...
async function addexpense(event)
{
    try {
        ...
        if(expense!=='' && expense!==undefined && description!=='' && description!==undefined && choice!=='' && choice!==undefined)
        {
            ...
            const token=localStorage.getItem("user");
                const adddata= await axios.post("http://13.230.79.168:4000/addexpense",expensedata,{headers: {"Authorization":token}});
...
window.addEventListener('DOMContentLoaded',async(event)=>{
    try {
        ...
        document.getElementById('username').innerText=`WELCOME ${LoginUser.toUpperCase()}`; 
       ...
       const expensedata=await axios.get(`http://13.230.79.168:4000/getexpensedata?param1=${pageno}&param2=${pageSize}`,{headers: {"Authorization":token}});
        console.log(expensedata);
...
async function deletexpense(expenseid)
{
    try {
        const token=localStorage.getItem("user");
        const deletestatus= await axios.delete(`http://13.230.79.168:4000/deleteExpenseData/${expenseid}`,{headers: {"Authorization":token}});
...
{ 
    ...
    try {
        ...
        {
            const token=localStorage.getItem("user");
            const expensedata=await axios.get("http://13.230.79.168:4000/getleaderboarddata",{headers: {"Authorization":token}});
  ...
  function download()
        {
            ...
            axios.get("http://13.230.79.168:4000/downloaddata",{headers: {"Authorization":token}})
            .then((response)=>{
...
async function downloadAllFile()
{
    try {
        ...
        {
                const token=localStorage.getItem("user");
            const DownloadFilelist= await axios.get("http://13.230.79.168:4000/downloaddataAllFile",{headers: {"Authorization":token}})
            .then((response)=>{
                    if(response.status===200){
...
async function getProducts (page){
    ...
    const expensedata=await axios.get(`http://13.230.79.168:4000/getexpensedata?param1=${page}&param2=${pageSize}`,{headers: {"Authorization":token}})
